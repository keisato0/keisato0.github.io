...
お嬢さん：フォーマライズすると、平均$\mu$、分散$\sigma^2$の母集団から得られた標本の標本平均$\bar{X}$の仮想的確率変数の期待値$E(\bar{X})$が$\mu$と一致するということですね、今までの話は。
...
お嬢さん：サンプルサイズが大きくなるほど$\bar{X}$の確率変数の分散は小さくなる、言い換えれば$\mu$近傍の値をとる可能性が高くなるということですね。

おばあさん：つまり、一回きりのサンプリングから得られた$\bar{X}$でも、$n$が大きければ$\mu$に近い可能性が高くなるので、$n$を増やせば推定の精度を上げられるわけです。

お嬢さん：$\bar{X}$の確率変数の確率密度関数とかヒストグラムで考えると、$n$が大きいほど真ん中の峰が高くて、両脇は締まったような形になるわけですね。

おばあさん：実際には$n$を増やすのは難しい場合も多いだろうが、やはり$n$は多いほうが$\mu$の正確な推定のためにはいいのぢや。

お嬢さん：なんかこれ、かなり直観通りの話ぢやない？
...
平均$\mu$、分散$\sigma^2$の母集団からサンプルサイズ$n$でサンプリングした際のサンプル内平均$\bar{X}$の仮想的な確率変数の標準化変数  
...
お嬢さん：中心極限定理で、$\bar{X}$が$t$分布に従うパターンもあるっていうようなことですかね。
